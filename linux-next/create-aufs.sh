...
rm -rf ../tmp/linux-3.9
mkdir -p ../tmp/linux-3.9
cp -a fs ../tmp/linux-3.9
cp -a include ../tmp/linux-3.9
cp -a Documentation ../tmp/linux-3.9
rm ../tmp/linux-3.9/include/uapi/linux/Kbuild
cd ../tmp
diff -Naur null linux-3.9  | filterdiff | \
sed -e 's|null\(/include/uapi/linux/Kbuild\)|linux-3.9-old\1|;s|^--- null.*|--- /dev/null|;\|linux-3.9/include/uapi/linux/Kbuild|,${\|@@ -0,0 +1 @@|,$d}' \
| bzip2 > aufs$(sed -ne 's|#define.*AUFS_VERSION.*"\(.*\)"|\1|p'  linux-3.9/include/linux/aufs_type.h).patch.bz2
mv *.bz2 $OLDPWD
cd $OLDPWD
mv *patch* ..
cd ..
