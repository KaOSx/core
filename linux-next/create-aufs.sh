...
rm -rf ../tmp/linux-5.12
mkdir -p ../tmp/linux-5.12
cp -a fs ../tmp/linux-5.12
cp -a include ../tmp/linux-5.12
cp -a Documentation ../tmp/linux-5.12
rm ../tmp/linux-5.12/include/uapi/linux/Kbuild
cd ../tmp
diff -Naur null linux-5.12  | filterdiff | \
sed -e 's|null\(/include/uapi/linux/Kbuild\)|linux-5.12-old\1|;s|^--- null.*|--- /dev/null|;\|linux-5.12/include/uapi/linux/Kbuild|,${\|@@ -0,0 +1 @@|,$d}' \
| bzip2 > aufs$(sed -ne 's|#define.*AUFS_VERSION.*"\(.*\)"|\1|p'  linux-5.12/include/uapi/linux/aufs_type.h).patch.bz2
mv *.bz2 $OLDPWD
cd $OLDPWD
mv *patch* ..
cd ..
