...
cd aufs4-standalone
git branch -r
git checkout origin/aufs4.20.4+
...
rm -rf ../tmp/linux-4.20
mkdir -p ../tmp/linux-4.20
cp -a fs ../tmp/linux-4.20
cp -a include ../tmp/linux-4.20
cp -a Documentation ../tmp/linux-4.20
rm ../tmp/linux-4.20/include/uapi/linux/Kbuild
cd ../tmp
diff -Naur null linux-4.20  | filterdiff | \
sed -e 's|null\(/include/uapi/linux/Kbuild\)|linux-4.20-old\1|;s|^--- null.*|--- /dev/null|;\|linux-4.20/include/uapi/linux/Kbuild|,${\|@@ -0,0 +1 @@|,$d}' \
| bzip2 > aufs$(sed -ne 's|#define.*AUFS_VERSION.*"\(.*\)"|\1|p'  linux-4.20/include/uapi/linux/aufs_type.h).patch.bz2
mv *.bz2 $OLDPWD
cd $OLDPWD
mv *patch* ..
cd ..
